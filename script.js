        ...
        try {
            ...
            auth.onAuthStateChanged(user => {
                ...
                if (user) {
                    ...
                    // Track message count states per channel to recognize increments for sounds / alerts
                    let initialLoadDone = false;
                    let lastMessageCount = 0;

                    rtdb.ref(`chats/chat_${user.uid}`).on("value", s => {
                        const chat = s.val();
                        if (chat) {
                            userChats = [chat];
                            saveData();
                            ...
                            const prevCount = initialLoadDone ? lastMessageCount : count;

                            if (initialLoadDone && count > prevCount) {
                                const latestMsg = msgs[count - 1];
                                // Alert only if the message came from an admin or supplier
                                if (latestMsg && latestMsg.senderRole === 'admin') {
                                    showNotificationToast("New Message from Admin", latestMsg.text || "Sent an attachment");
                                    try {
                                        // Play generic clean notification chime
                                        const audio = new Audio('https://assets.mixkit.co/active_storage/sfx/2869/2869-600.wav');
                                        audio.volume = 0.8;
                                        audio.play().catch(e => console.log("Audio autoplay deferred until interaction:", e));
                                    } catch (audioErr) {
